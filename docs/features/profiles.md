...
Vision capability is catalog-derived. The bundled model registry carries an
explicit `supports_vision` value for each model, and image input is allowed
only when that value is `true`; it is not a profile-level override.
