...
const PaletteContent: FC<{
  ...
  return (
    <>
      ...
      <ComposerPrimitive.Unstable_TriggerPopoverItems className="max-h-[min(22rem,45vh)] overflow-y-auto overscroll-contain pr-0.5">
        {(visibleItems) =>
          visibleItems.length === 0 ? (
            ...
            visibleItems.map((item) => (
              <ComposerPrimitive.Unstable_TriggerPopoverItem
                key={item.id}
                item={item}
                className="data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground group flex w-full items-center gap-3 rounded-xl px-2.5 py-2 text-left transition-colors hover:bg-accent/70"
              >
