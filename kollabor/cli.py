...
async def _handle_cli_attach(identity: str) -> None:
    """Handle --attach flag: live TUI proxy to a running agent.

    Args:
        identity: The hub identity of the agent to attach to.
    """
    import json
    import os
    ...
    # Scan presence files for the target identity
    target_socket = None
        ...
        try:
            ...
            if data.get("identity") == identity:
                ...
                try:
                    ...
                    continue
                target_socket = data.get("socket_path", "")
                target_identity = data.get("identity", identity)
                break
        except Exception:
            continue

    if not target_socket:
        print(f"no agent found with identity '{identity}'", file=sys.stderr)
        ...
        for f in presence_dir.glob("*.json"):
            try:
                ...
                try:
                    ...
                    continue
                available.append(data.get("identity", "?"))
            except Exception:
                continue
        ...
        if available:
            ...
            print("no agents currently online", file=sys.stderr)
        sys.exit(1)

    # Launch live attach client
    from kollabor.attach_client import AttachClient

    if not target_socket or not target_identity:
        print(f"no agent found with identity '{identity}'", file=sys.stderr)
        sys.exit(1)
    interactive = "--interactive" in sys.argv or "-i" in sys.argv
    client = AttachClient(target_socket, target_identity, interactive=interactive)
    await client.run()

    print(f"\n-- end of {target_identity} output --")
