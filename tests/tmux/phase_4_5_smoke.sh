...
# attaches a client with --provider openai-oauth. After the attach settles,
...
# ---------------------------------------------------------------------------
# Step 2: Attach with --provider openai-oauth (THE critical test)
# ---------------------------------------------------------------------------
log "step 2: attaching client with --provider openai-oauth"

tmux -L "$TMUX_SOCKET" new-session -d -s "$ATTACH_SESSION" -x 140 -y 40 \
    "python main.py --attach $IDENTITY --provider openai-oauth"
...
# Assertion 4: No "--provider openai-oauth failed on daemon" error
