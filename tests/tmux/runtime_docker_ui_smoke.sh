...
test_oauth_cache() {
    ...
    start_session "$session" "$container" "$volume" none kollab --provider openai-oauth --no-daemon || return
...
test_zai_profile() {
    ...
    start_session "$session" "$container" "$volume" none kollab --provider default --no-daemon || return
...
test_tools() {
    ...
    start_session "$session" "$container" "$volume" none kollab --provider "$WORKING_PROFILE" --no-daemon || return
...
test_hub() {
    ...
    start_session "$primary" "$primary_container" "$volume" none kollab --provider "$WORKING_PROFILE" --no-daemon || return
    start_session "$peer" "$peer_container" "$volume" none kollab --agent coder --as lapis --provider "$WORKING_PROFILE" --no-daemon || return
