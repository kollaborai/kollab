...
def _fresh_feed(tmp_path: Path) -> ChangeFeed:
    return ChangeFeed(hub_dir=str(tmp_path))


def test_claim_then_release_all_clears_identity(tmp_path: Path):
    """Baseline: release_all removes every claim held by the identity."""
    feed = _fresh_feed(tmp_path)
...
def test_after_release_spawn_precheck_does_not_see_stale_claim(tmp_path: Path):
...
def test_release_all_on_unknown_identity_is_safe(tmp_path: Path):
...
def test_release_all_persists_to_disk(tmp_path: Path):
